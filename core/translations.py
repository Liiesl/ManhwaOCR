...
from PyQt5.QtWidgets import (QDialog, QVBoxLayout, QTextEdit, QProgressBar, QShortcut,
                             QPushButton, QHBoxLayout, QMessageBox, QLabel, QSplitter,
                             QScrollArea, QFrame, QWidget, QCheckBox, QGridLayout,
                             QComboBox)
from PyQt5.QtCore import QThread, pyqtSignal, Qt, QSize, QTimer, QEvent
        ...
        try:
            ...
            for chunk in response_stream:
                ...
                if chunk.text:
                    ...
                    self.translation_progress.emit(chunk.text)
...
class TranslationWindow(QDialog):
    ...
    def eventFilter(self, source, event):
        if event.type() == QEvent.MouseButtonPress and source in self.clickable_frames:
            ...
            if modifiers == Qt.ShiftModifier:
                ...
                # BUGFIX 2: This is a compound action (select one, deselect others).
                # Relying on a storm of signals is fragile and was causing the master
                # checkbox to show an incorrect state. We now make this atomic by
                # blocking signals and manually updating the UI afterwards.
    ...
    def _on_select_all_changed(self, state):
        ...
        # BUGFIX 3: The original logic had a faulty guard that ignored the user's first click
        # on a tristate checkbox because it would enter a `PartiallyChecked` state.
        # This new logic correctly interprets any user click that doesn't result in an
        # `Unchecked` state as a command to select all rows.
        if self.select_all_checkbox.sender() is not self.select_all_checkbox:
            # This is a programmatic change, likely from _update_select_all_checkbox_state
            # which already blocks signals. This is an extra safeguard.
            return
    ...
    def _update_comparison_panel(self, column_index, parsed_data):
        ...
        # BUGFIX 1: Save the parsed data to the backing data model.
        # This prevents translations from disappearing when the grid is rebuilt.
        ...
        try:
            ...
            for col_data in self.translation_columns:
                ...
                for result in self.ocr_results:
                    ...
                    text_to_apply = self._get_text_for_profile(result, source_profile_name)
